...
public class BlackJackController implements Initializable {
	...
	private PathTransition CreatePathTransition(Point2D fromPoint, Point2D toPoint, ImageView img) {
		...
		path.getElements().add(new CubicCurveTo(toPoint.getX() * 6, toPoint.getY() * 1, toPoint.getX() / 3,
				toPoint.getY() / 5, toPoint.getX(), toPoint.getY()));
	...
	private RotateTransition CreateRotateTransition(ImageView img) {
		...
		RotateTransition rotateTransition = new RotateTransition(Duration.millis(iAnimationLength / 1), img);
